...
<x-modal name="detalhes-emprestimo-modal" title="Detalhes do Empréstimo" maxWidth="w-11/12 md:w-3/4 lg:w-1/2">
    <div class="space-y-4">
        ...
        <div class="flex justify-end gap-2 pt-2">
            <x-button type="button" color="secondary" onclick="window.location='{{ route('biblioteca.emprestimos.index') }}'">Fechar</x-button>

            @if($emprestimo->status === 'ativo')
                <form action="{{ route('biblioteca.emprestimos.devolver', $emprestimo) }}" method="POST" onsubmit="return confirm('Confirmar devolução deste empréstimo?')">
                    @csrf
                    @method('PATCH')
                ...
                @if($emprestimo->data_prevista && $emprestimo->data_prevista >= now())
                    <form action="{{ route('biblioteca.emprestimos.renovar', $emprestimo) }}" method="POST" onsubmit="return confirm('Confirmar renovação deste empréstimo?')">
                        @csrf
                        @method('PATCH')
