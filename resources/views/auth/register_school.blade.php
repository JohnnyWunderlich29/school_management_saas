        ...
        <div class="p-6">
            ...
            <form method="POST" action="{{ route('register.escola.submit') }}" class="space-y-8">
                ...
                <section class="wizard-step" data-step="1">
                    ...
                    <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="md:col-span-1">
                            <x-input name="escola_nome" id="escola_nome" label="Nome da Escola" :value="old('escola_nome')" placeholder="Ex.: Escola Modelo" required />
                        </div>
                        <div>
                            <x-input name="cnpj" id="cnpj" label="CNPJ" :value="old('cnpj')" placeholder="00.000.000/0000-00" inputmode="numeric" required />
                        </div>
                        <div>
                            <x-input name="escola_email" id="escola_email" label="Email da Escola" type="email" :value="old('escola_email')" placeholder="contato@escola.com.br" required />
                        </div>
                        <div>
                            <x-input name="telefone" id="telefone" label="Telefone" :value="old('telefone')" placeholder="(00) 0000-0000" inputmode="numeric" />
                        </div>
                        <div>
                            <x-input name="celular" id="celular" label="Celular" :value="old('celular')" placeholder="(00) 00000-0000" inputmode="numeric" required />
                        </div>
                        <div>
                            <x-input name="cep" id="cep" label="CEP" :value="old('cep')" placeholder="00000-000" class="pr-12" inputmode="numeric" required help="Digite o CEP e clique na lupa para buscar.">
                                <button type="button" id="btn-buscar-cep" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-indigo-600" title="Buscar CEP" aria-label="Buscar CEP">
                                    <svg id="icon-cep" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="md:col-span-2">
                            <x-input name="endereco" id="endereco" label="Endereço" :value="old('endereco')" placeholder="Rua, Avenida..." />
                        </div>
                        <div>
                            <x-input name="numero" id="numero" label="Número" :value="old('numero')" placeholder="Nº" />
                        </div>
                        <div>
                            <x-input name="complemento" id="complemento" label="Complemento" :value="old('complemento')" placeholder="Apto, Bloco..." />
                        </div>
                        <div>
                        ...
                        <div>
                            <x-input name="estado" id="estado" label="Estado (UF)" :value="old('estado')" maxlength="2" placeholder="UF" />
                        </div>
                    </div>

                    <div class="mt-6 flex justify-end">
                        <button type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md shadow hover:bg-indigo-700 focus:outline-none" onclick="nextStep()">Próximo</button>
                    </div>
                </section>
                ...
                <section class="wizard-step hidden" data-step="2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach ($plans as $plan)
                                ...
                                <label for="plan_{{ $plan->id }}" class="cursor-pointer">
                                    <input id="plan_{{ $plan->id }}" name="plan_id" type="radio" value="{{ $plan->id }}" class="sr-only peer" data-price="{{ number_format($planPrice, 2, '.', '') }}" data-plan-name="{{ $plan->name }}" {{ (string)old('plan_id') === (string)$plan->id ? 'checked' : '' }} required aria-describedby="plan_desc_{{ $plan->id }}">
                                    <div class="relative h-full rounded-2xl ring-1 {{ $isRecommended ? 'ring-indigo-300 bg-indigo-50' : 'ring-gray-200 bg-white' }} p-6 shadow-sm transition peer-checked:ring-2 peer-checked:ring-indigo-600">
                                        @if($isRecommended)
                                            <span class="absolute -top-3 left-4 inline-flex items-center rounded-full bg-indigo-600 px-2 py-0.5 text-xs font-medium text-white shadow">Mais popular</span>
                                        @endif
                                        @if($plan->is_trial)
                                            <span class="absolute -top-3 right-4 inline-flex items-center rounded-full bg-amber-500 px-2 py-0.5 text-xs font-medium text-white shadow">Trial {{ $plan->trial_days }} dias</span>
                                        @endif
                                        <div class="flex flex-col h-full">
                                            <div>
                                                <h3 class="text-base font-semibold {{ $isRecommended ? 'text-indigo-900' : 'text-gray-900' }}">{{ $plan->name }}</h3>
                                                <p id="plan_desc_{{ $plan->id }}" class="mt-1 text-sm {{ $isRecommended ? 'text-indigo-700' : 'text-gray-500' }}">{{ $plan->description }}</p>
                                                <div class="mt-4 flex items-baseline gap-1">
                                                    <span class="text-3xl font-bold {{ $isRecommended ? 'text-indigo-900' : 'text-gray-900' }}">R$ {{ number_format((float)$plan->price, 2, ',', '.') }}</span>
                                                    <span class="text-sm {{ $isRecommended ? 'text-indigo-700' : 'text-gray-500' }}">/mês</span>
                                                </div>
                                                <div class="mt-1 text-xs {{ $isRecommended ? 'text-indigo-700' : 'text-gray-500' }}">Usuários: {{ $plan->max_users ?? '-' }} · Alunos: {{ $plan->max_students ?? '-' }}</div>
                                            </div>
                                            <ul class="mt-6 space-y-2 text-sm {{ $isRecommended ? 'text-indigo-900' : 'text-gray-700' }}">
                                                <li class="flex items-start gap-2"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg> Inclui módulos essenciais</li>
                                                <li class="flex items-start gap-2"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg> Suporte padrão</li>
                                            </ul>
                                            <div class="mt-6">
                                                <span class="inline-flex items-center justify-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 peer-checked:bg-indigo-700">Selecionar</span>
                                            </div>
                                        </div>
                                    </div>
                                </label>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 rounded-xl ring-1 ring-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    @foreach($plans as $plan)
                                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">{{ $plan->name }}</th>
                                    @endforeach
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 bg-white">
                                ...
                                <tr class="bg-gray-50">
                                    ...
                                    @foreach($plans as $plan)
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ $plan->max_students ?? '-' }}</td>
                                    @endforeach
                                </tr>
                                <tr>
                                    <td class="px-4 py-2 text-sm text-gray-700">Período trial</td>
                                    @foreach($plans as $plan)
                                        <td class="px-4 py-2 text-sm text-gray-900">{{ $plan->is_trial ? ($plan->trial_days.' dias') : '—' }}</td>
                                    @endforeach
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="mt-8">
                        <h3 class="text-sm font-medium text-gray-900">Módulos essenciais incluídos em todos os planos</h3>
                        <ul class="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                            @forelse ($essentialModules as $module)
                                <li class="flex items-center gap-2 text-sm text-gray-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                    ...
                    <div class="mt-8">
                        ...
                        <div class="mt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            @forelse ($optionalModules as $module)
                                ...
                                <label class="group cursor-pointer relative rounded-xl ring-1 ring-gray-200 bg-white p-4 shadow-sm hover:shadow transition">
                                    <div class="flex items-center gap-3">
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2">
                                                <span class="text-sm font-medium text-gray-900">{{ $module->display_name ?? $module->name }}</span>
                                                @if($modulePrice > 0)
                                                    <span class="ml-auto text-xs text-gray-500">R$ {{ number_format($modulePrice, 2, ',', '.') }}/mês</span>
                                                @else
                                                    <span class="ml-auto text-xs text-gray-500">Gratuito</span>
                                                @endif
                                            </div>
                                            @if(!empty($module->description))
                                                <p id="module_desc_{{ $module->id }}" class="mt-1 text-xs text-gray-500">{{ $module->description }}</p>
                                            @endif
                                        </div>
                                        <input type="checkbox" name="modules[]" value="{{ $module->id }}" class="sr-only peer" data-price="{{ number_format($modulePrice, 2, '.', '') }}" @checked(in_array($module->id, old('modules', []))) aria-describedby="module_desc_{{ $module->id }}">
                                        <div class="w-10 h-6 rounded-full bg-gray-200 relative transition peer-checked:bg-indigo-600 ring-1 ring-gray-300">
                                            <span class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full shadow transform transition peer-checked:translate-x-4"></span>
                                        </div>
                                    </div>
                                </label>
                            @empty
                    ...
                    <div id="pricing-summary" class="mt-8 rounded-xl ring-1 ring-gray-200 bg-white p-4" aria-live="polite">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="text-right">
                                ...
                                <div id="summary-breakdown" class="text-xs text-gray-500">Plano: R$ 0,00 + Addons: R$ 0,00</div>
                            </div>
                        </div>
                    </div>

                    <div class="mt-6 flex justify-between">
                        <button type="button" class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-md shadow hover:bg-gray-200 focus:outline-none" onclick="prevStep()">Voltar</button>
                        <button type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md shadow hover:bg-indigo-700 focus:outline-none" onclick="nextStep()">Próximo</button>
                    </div>
                </section>
                ...
                <section class="wizard-step hidden" data-step="3">
                    ...
                    <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <x-input name="admin_email" id="admin_email" label="Email" type="email" :value="old('admin_email')" required />
                        <x-input name="password" id="password" label="Senha" type="password" required />
                        <x-input name="password_confirmation" id="password_confirmation" label="Confirmar Senha" type="password" required />
                    </div>

                    <div class="mt-6 flex justify-between">
                        <button type="button" class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-md shadow hover:bg-gray-200 focus:outline-none" onclick="prevStep()">Voltar</button>
                        <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">Registrar Escola</button>
                    </div>
                </section>
            </form>
        </div>
...
<script>
    ...
    function nextStep() {
        if (currentStep === 1) {
            if (typeof window.validateStep1 === 'function') {
                if (!window.validateStep1()) { return; }
            }
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function showFieldError(inputEl, id, msg){
            ...
            if(!box) return;
            if(!msg){ box.classList.add('hidden'); box.innerHTML=''; inputEl?.removeAttribute('aria-invalid'); return; }
        ...
        function sanitizeCNPJ(v){ return (v||'').replace(/\D/g,'').slice(0,14); }
        function isValidCNPJ(v){
            ...
            const calcDV = (base, pesos) => {
                let s = 0; for(let i=0;i<pesos.length;i++){ s += parseInt(base[i],10) * pesos[i]; }
        ...
        function validateStep1(){
            ...
            if(!escolaNomeInput || !escolaNomeInput.value.trim()){
                ...
                ok = false; if(escolaNomeInput) focusQueue.push(escolaNomeInput);
            } else { showFieldError(escolaNomeInput, 'escola-nome-alert', ''); }
            // CNPJ
            if(!cnpjInput || !isValidCNPJ(cnpjInput.value)){
                showFieldError(cnpjInput, 'cnpj-alert-box', 'CNPJ inválido. Verifique os dígitos.');
                ok = false; if(cnpjInput) focusQueue.push(cnpjInput);
            } else { showFieldError(cnpjInput, 'cnpj-alert-box', ''); }
            // Email da escola
            ...
            if(!emailOk){
                ...
                ok = false; if(escolaEmailInput) focusQueue.push(escolaEmailInput);
            } else { showFieldError(escolaEmailInput, 'escola-email-alert', ''); }
            // Celular (11 dígitos)
            ...
            if(celDigits.length !== 11){
                ...
                ok = false; if(celularInput) focusQueue.push(celularInput);
            } else { showFieldError(celularInput, 'celular-alert', ''); }
            // CEP (8 dígitos)
            const cepDigits = (cepInput?.value||'').replace(/\D/g,'');
            if(cepDigits.length !== 8){
                showCepAlert('error','Informe um CEP válido com 8 dígitos.');
                ok = false; if(cepInput) focusQueue.push(cepInput);
            } else { showCepAlert(null,''); }

            if(!ok && focusQueue.length){ focusQueue[0].focus(); }
            return ok;
        }
        ...
        if(celularInput){
            celularInput.addEventListener('input', () => { celularInput.value = maskPhone(celularInput.value); });
        }
        if(cnpjInput){
            cnpjInput.addEventListener('input', () => { cnpjInput.value = maskCNPJ(cnpjInput.value); });
            cnpjInput.addEventListener('blur', () => {
                if(cnpjInput.value.trim() === ''){ showFieldError(cnpjInput, 'cnpj-alert-box', ''); return; }
        ...
        if(estadoInput){
            estadoInput.addEventListener('input', () => { estadoInput.value = (estadoInput.value || '').toUpperCase().slice(0,2); });
        }
        ...
        function updatePricingSummary(){
            ...
            summaryBreakdownEl.textContent = `Plano: ${currencyFmt.format(plan)} + Addons: ${currencyFmt.format(addons)}`;
        }
        ...
        const form = document.querySelector('form[action*="register.escola.submit"]') || document.querySelector('form');
        if(form){
