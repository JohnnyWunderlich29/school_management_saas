...
@section('content')
    ...
    <x-modal name="justificar-ausencia-modal" title="Justificar Ausência" maxWidth="max-w-md">
        <form id="justificativaForm">
            ...
            <div class="mb-4">
                ...
                <textarea id="justificativa" name="justificativa" rows="3" maxlength="255"
                    class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full text-sm border-gray-300 rounded-md resize-none"
        ...
        <x-slot name="footer">
            ...
            <x-button type="button" id="salvarJustificativa" color="primary"><i
                    class="fas fa-save mr-1.5"></i>Salvar</x-button>
        </x-slot>
    </x-modal>
    ...
    <x-card>
        ...
        <x-collapsible-filter title="Filtros de Lançamento" :action="route('presencas.lancar')" :clear-route="route('presencas.lancar')"
            ...
            <x-filter-field name="tempo_aula" label="Tempo de Aula" type="select" emptyOption="Todos os tempos"
                :options="[
                    '1' => '1º Tempo',
                    '2' => '2º Tempo',
                    '3' => '3º Tempo',
            ...
            <x-date-filter-with-arrows name="data_inicio" label="Data Início" :value="$dataInicio" data-fim-name="data_fim"
        ...
        <div class="mt-6 mb-6 rounded-lg border border-gray-200 bg-gray-50 p-4 md:p-5">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div class="flex flex-wrap items-center gap-2 md:gap-3">
                    <span
                        class="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs md:text-sm font-medium bg-green-100 text-green-800 border border-green-200">
                        <i class="fas fa-check mr-1.5"></i> Presente
                    </span>
                    <span
                        class="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs md:text-sm font-medium bg-red-100 text-red-800 border border-red-200">
                        <i class="fas fa-times mr-1.5"></i> Ausente
                    </span>
                    <span
                        class="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs md:text-sm font-medium bg-gray-100 text-gray-800 border border-gray-200">
                        <i class="fas fa-circle mr-1.5 text-[10px]"></i> Pendente
                    </span>
                    <span
                        class="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs md:text-sm font-medium bg-yellow-50 text-yellow-700 border border-yellow-200">
                        <i class="fas fa-ban mr-1.5"></i> Fora da grade
                    </span>
                </div>
                <p class="text-xs md:text-sm text-gray-600">Dica: use os botões abaixo de cada data para aplicar o status em
                    todos os alunos daquele dia.</p>
            </div>
        </div>
            ...
            <div data-ajax-content>
                @if ($salas->count() > 0)
                    ...
                    --}}
                            ...
                            <div class="hidden md:block space-y-6">
                                @foreach ($datas as $dataInfo)
                                    ...
                                    <div class="overflow-x-auto bg-white rounded-lg border border-gray-200">
                                        ...
                                        <div
                                            class="px-4 py-3 border-b flex items-center justify-between {{ $dataInfo['eh_fim_semana'] ? 'bg-yellow-50' : 'bg-gray-50' }} {{ $dataInfo['eh_hoje'] ? 'ring-1 ring-blue-200 bg-blue-50' : '' }}">
                                            <div class="flex flex-col">
                                                <span
                                                    class="font-semibold text-sm md:text-base {{ $dataInfo['eh_hoje'] ? 'text-blue-700' : 'text-gray-800' }}">{{ \Carbon\Carbon::parse($dataKey)->format('d/m') }}</span>
                                                <span
                                                    class="text-xs {{ $dataInfo['eh_fim_semana'] ? 'text-orange-600' : 'text-gray-500' }}">{{ \Carbon\Carbon::parse($dataKey)->locale('pt_BR')->dayName }}</span>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                <button type="button"
                                                    ...
                                                    data-data="{{ $dataKey }}" data-sala-id="{{ $sala->id }}"
                                                    data-presente="1"
                                        ...
                                        <table class="min-w-full table-fixed divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th
                                                        class="px-6 py-3 text-left text-xs md:text-sm font-medium text-gray-600 uppercase tracking-wider">
                                                        Aluno</th>
                                                    @for ($t = 1; $t <= 5; $t++)
                                                        <th
                                                            class="px-3 py-3 text-center text-xs md:text-sm font-medium text-gray-600 uppercase tracking-wider w-20">
                                                            {{ $t }}º Tempo</th>
                                                    @endfor
                                                </tr>
                                            </thead>
                                                ...
                                                @foreach ($sala->alunos as $aluno)
                                                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                                                            ...
                                                            <div class="flex items-center">
                                                                <div class="flex-shrink-0 h-10 w-10">
                                                                    <div
                                                                        ...
                                                                        <span
                                                                            class="text-sm font-medium text-white">{{ strtoupper(substr($aluno->nome, 0, 2)) }}</span>
                                                                    </div>
                                                                </div>
                                                                <div class="ml-4">
                                                                    <div class="text-sm font-medium text-gray-900">
                                                                        {{ $aluno->nome }} {{ $aluno->sobrenome }}</div>
                                                                    <div class="text-sm text-gray-500">Matrícula:
                                                                        {{ $aluno->matricula ?? $aluno->id }}</div>
                                                                    <div class="mt-2">
                                                                        <button type="button"
                                                                            ...
                                                                            data-data="{{ $dataKey }}"
                                                                            data-sala-id="{{ $sala->id }}">
                                                                            <i class="fas fa-user-slash mr-1"></i> Marcar
                                                                            turno como F
                                                                        </button>
                                                                        <button type="button"
                                                                            ...
                                                                            data-data="{{ $dataKey }}"
                                                                            data-sala-id="{{ $sala->id }}">
                                                                            <i class="fas fa-user-check mr-1"></i> Marcar
                                                                            turno como P
                                                                        </button>
                                                                    </div>
                                                                </div>
                                                        ...
                                                        @php
                                                            $alunoId = $aluno->id;
                                                            $temposPermitidos =
                                                                $temposDisponiveis[$sala->id][$dataKey] ?? [];
                                                        @endphp
                                                        @for ($t = 1; $t <= 5; $t++)
                                                            @php
                                                                $presencaTempo =
                                                                    $presencasFormatadas[$dataKey][$alunoId][$t] ??
                                                                    null;
                                                            ...
                                                            <td class="px-3 py-3 text-center align-middle">
                                                                @if ($presencaTempo)
                                                                    <div class="flex items-center justify-center gap-2">
                                                                        <span
                                                                            class="inline-flex items-center px-2 py-1 rounded-full text-[12px] font-medium {{ $presencaTempo->presente ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200' }}">
                                                                            <i
                                                                                class="fas fa-{{ $presencaTempo->presente ? 'check' : 'times' }} mr-1"></i>{{ $presencaTempo->presente ? 'P' : 'F' }}
                                                                        </span>
                                                                ...
                                                                @else
                                                                    <div class="flex items-center justify-center gap-1">
                                                                        <button type="button"
                                                                            ...
                                                                            data-sala-id="{{ $sala->id }}"
                                                                            data-tempo-aula="{{ $t }}"
                                                                            data-presente="1" title="Marcar presente"
                                                                        ...
                                                                        <button type="button"
                                                                            ...
                                                                            data-sala-id="{{ $sala->id }}"
                                                                            data-tempo-aula="{{ $t }}"
                                                                            data-presente="0" data-bs-toggle="modal"
                                                                            data-bs-target="#justificativaModal"
                            ...
                            @if ($sala->alunos instanceof \Illuminate\Pagination\LengthAwarePaginator)
                            ...
                            <div class="md:hidden space-y-4">
                                @foreach ($sala->alunos as $aluno)
                                    <div
                                        class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow duration-200 mb-4">
                                        <!-- Header do card -->
                                        <div class="flex items-center mb-4 pb-3 border-b border-gray-100">
                                            <div
                                                class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white mr-3 flex-shrink-0 shadow-sm">
                                                <span class="text-sm font-medium">
                                                    {{ strtoupper(substr($aluno->nome, 0, 2)) }}
                                                </span>
                                            </div>
                                            <div class="flex-1">
                                                <h3 class="font-semibold text-gray-900 text-base">{{ $aluno->nome }}
                                                    {{ $aluno->sobrenome }}</h3>
                                            ...
                                            <div class="flex flex-col w-full md:flex-row gap-4 mb-6">
                                                @php
                                                    ...
                                                    foreach ($datas as $dataInfo) {
                                                        ...
                                                        $recordedTemps = array_filter(
                                                            array_keys($presencasDoDia),
                                                            function ($t) {
                                                                return is_numeric($t) && $t >= 1 && $t <= 5;
                                                            },
                                                        );
                                                        $recordedInPermitted = count(
                                                            array_intersect($temposPermitidos, $recordedTemps),
                                                        );
                                                        $naoRegistrados += max(
                                                            0,
                                                            count($temposPermitidos) - $recordedInPermitted,
                                                        );
                                                    }
                                                @endphp
                                                <div
                                                    class="bg-green-50 rounded-xl p-4 text-center border border-green-100 shadow-sm">
                                                    <div class="text-lg font-bold text-green-700 mb-1">{{ $presentes }}
                                                    </div>
                                                    <div class="text-xs font-medium text-green-600">Presentes</div>
                                                </div>
                                                <div
                                                    class="bg-red-50 rounded-xl p-4 text-center border border-red-100 shadow-sm">
                                                    <div class="text-lg font-bold text-red-700 mb-1">{{ $ausentes }}
                                                    </div>
                                                    <div class="text-xs font-medium text-red-600">Ausentes</div>
                                                </div>
                                                <div
                                                    class="bg-yellow-50 rounded-xl p-4 text-center border border-yellow-100 shadow-sm">
                                        ...
                                        <div class="space-y-2">
                                            @foreach ($datas as $data)
                                                ...
                                                <div
                                                    class="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 bg-gray-50 rounded-lg gap-3">
                                                    <div class="text-sm font-medium text-gray-700 flex-1">
                                                        <div class="flex items-center gap-2">
                                                            <span
                                                                class="font-semibold {{ $isHoje ? 'text-blue-700' : 'text-gray-800' }}">{{ $carbon->format('d/m') }}</span>
                                                            <span
                                                                class="text-xs {{ $isFimSemana ? 'text-orange-600' : 'text-gray-500' }}">{{ $dayName }}</span>
                                                        </div>
                                                        <div class="mt-2">
                                                            <button type="button"
                                                                class="marcar-turno-f-aluno-btn inline-flex items-center px-2 py-1 rounded text-[11px] font-medium bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 hover:border-red-300 transition-all duration-150 focus:outline-none focus:ring-1 focus:ring-red-500"
                                                    ...
                                                    <div
                                                        class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 w-full sm:ml-4">
                                                        @for ($t = 1; $t <= 5; $t++)
                                                            @php
                                                                $presencaTempo =
                                                                    $presencasFormatadas[$dataKey][$alunoId][$t] ??
                                                            ...
                                                            <div class="flex flex-col items-center">
                                                                <div class="text-xs leading-tight text-gray-600 mb-1">
                                                                    {{ $t }}º</div>
                                                                @if ($presencaTempo)
                                                                    <div class="flex items-center gap-2">
                                                                        <span
                                                                            class="inline-flex items-center px-2 py-1 rounded-full text-[12px] font-medium {{ $presencaTempo->presente ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                                            <i
                                                                                class="fas fa-{{ $presencaTempo->presente ? 'check' : 'times' }} mr-1"></i>{{ $presencaTempo->presente ? 'P' : 'F' }}
                                                                        </span>
                                                                        @if ($presencaTempo->presente)
                                                                ...
                                                                @else
                                                                    <div class="flex gap-1">
                                                                        <button type="button"
                                                                            ...
                                                                            data-sala-id="{{ $sala->id }}"
                                                                            data-tempo-aula="{{ $t }}"
                                                                            data-presente="1" title="Marcar como presente"
                                                                            {{ $desabilitado ? 'disabled' : '' }}>
                                                                            <i class="fas fa-check"></i>
                                                                        </button>
                                                                        <button type="button"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.presenca-btn').forEach(button => {
                button.addEventListener('click', async function() {
                    if (isBulkProcessing)
                        return; // bloquear clique individual durante processamento em massa
                    const alunoId = this.dataset.alunoId;
                    const data = this.dataset.data;
                    const presente = this.dataset.presente === '1';
                    const tempoAula = parseInt(this.dataset.tempoAula || this.getAttribute(
                        'data-tempo-aula'));
            ...
            document.querySelectorAll('.marcar-todos-btn').forEach(button => {
                button.addEventListener('click', async function() {
                    ...
                    const targetButtons = Array.from(document.querySelectorAll(
                            `.presenca-btn[data-data="${data}"][data-sala-id="${salaId}"]`))
                        .filter(btn => !btn.hasAttribute('disabled'));

                    // Desabilitar controles durante processamento
                    document.querySelectorAll('.marcar-todos-btn, .marcar-todos-btn-ausente')
                        .forEach(b => {
                            b.disabled = true;
                            b.classList.add('opacity-60');
                        });
                    document.querySelectorAll('.presenca-btn, .presenca-btn-ausencia').forEach(
                        b => {
                            b.disabled = true;
                        });
                    ...
                    try {
                        for (const btn of targetButtons) {
                            ...
                            const tempoAula = parseInt(btn.dataset.tempoAula || btn
                                .getAttribute('data-tempo-aula'));
                        ...
                        successMsg.innerHTML =
                            '<i class="fas fa-check-circle mr-2"></i>Todos marcados como presentes!';
                        document.body.appendChild(successMsg);
                        setTimeout(() => {
                            successMsg.remove();
                        }, 1200);
                        ...
                        document.querySelectorAll(
                            '.marcar-todos-btn, .marcar-todos-btn-ausente').forEach(b => {
                            b.disabled = false;
                            b.classList.remove('opacity-60');
                        });
                        document.querySelectorAll('.presenca-btn, .presenca-btn-ausencia')
                            .forEach(b => {
                                b.disabled = false;
                            });
                    }
            ...
            document.querySelectorAll('.marcar-todos-btn-ausente').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const absenceButtons = Array.from(document.querySelectorAll(
                        `.presenca-btn-ausencia[data-data="${data}"]`)).filter(btn => !btn
                        .hasAttribute('disabled'));
                    const editButtonsF = Array.from(document.querySelectorAll(
                        `.editar-presenca-btn[data-data="${data}"]`)).filter(btn => !btn
                        .hasAttribute('disabled'));
                    ...
                    const items = targetButtons.map(btn => {
                        const alunoId = btn.getAttribute('data-aluno-id') || btn.dataset
                            .alunoId;
                        const tempo = btn.getAttribute('data-tempo-aula') || btn.dataset
                            .tempoAula;
                        const slotContainer = btn.parentElement;
                        if (slotContainer && !slotContainer.hasAttribute(
                                'data-original-content')) {
                            slotContainer.setAttribute('data-original-content',
                    ...
                    document.querySelectorAll(
                        '.presenca-btn, .presenca-btn-ausencia, .editar-presenca-btn').forEach(
                        b => {
                            b.disabled = true;
                        });
                    ...
                    window.dispatchEvent(new CustomEvent('open-modal', {
                        detail: 'justificar-ausencia-modal'
                    }));
                });
            });
            ...
            document.addEventListener('click', function(e) {
                ...
                const absenceButtons = Array.from(document.querySelectorAll(
                        `.presenca-btn-ausencia[data-aluno-id="${alunoId}"][data-data="${data}"]`))
                    .filter(btn => !btn.hasAttribute('disabled'));
                const editButtonsF = Array.from(document.querySelectorAll(
                        `.editar-presenca-btn[data-aluno-id="${alunoId}"][data-data="${data}"]`))
                    .filter(btn => !btn.hasAttribute('disabled'));
                const targetButtons = [...absenceButtons, ...editButtonsF];
                ...
                const slotsMapF = Array.from(tempoToSlotsF, ([tempo, slots]) => ({
                    tempo,
                    slots
                }));
            ...
            document.addEventListener('click', async function(e) {
                ...
                const pButtons = Array.from(document.querySelectorAll(
                    `.trocar-para-p-btn[data-aluno-id="${alunoId}"][data-data="${data}"]`));
                const presencaButtons = Array.from(document.querySelectorAll(
                    `.presenca-btn[data-aluno-id="${alunoId}"][data-data="${data}"]`)).filter(btn =>
                    !btn.hasAttribute('disabled'));
                const targetButtons = [...pButtons, ...presencaButtons];
                const tempos = Array.from(new Set(targetButtons.map(btn => btn.getAttribute(
                    'data-tempo-aula') || btn.dataset.tempoAula).filter(Boolean)));
                ...
                const slotsMapP = Array.from(tempoToSlotsP, ([tempo, slots]) => ({
                    tempo,
                    slots
                }));
                ...
                try {
                    ...
                    successMsg.className =
                        'fixed top-4 right-4 bg-green-600 text-white px-4 py-2 rounded shadow-lg z-50';
                    successMsg.innerHTML =
                        `<i class="fas fa-check-circle mr-2"></i>Turno marcado como P! (${processedCount}/${totalCount} tempos processados)`;
                    document.body.appendChild(successMsg);
                    setTimeout(() => {
                        successMsg.remove();
                    }, 1500);
                    ...
                    if (massAlunoPContext && massAlunoPContext.slotsMap) {
                        massAlunoPContext.slotsMap.forEach(entry => {
                            ...
                            slotsArr.forEach(slotContainer => {
                                if (slotContainer && slotContainer.hasAttribute(
                                        'data-original-content')) {
                                    slotContainer.innerHTML = slotContainer
                                        .getAttribute('data-original-content');
                                    slotContainer.removeAttribute(
                                        'data-original-content');
                                }
                            });
                        });
            ...
            window.cancelJustificativaModal = function() {
                ...
                if (massAlunoFContext && massAlunoFContext.slotsMap) {
                    massAlunoFContext.slotsMap.forEach(entry => {
                        ...
                        slotsArr.forEach(slotContainer => {
                            if (slotContainer && slotContainer.hasAttribute(
                                    'data-original-content')) {
                                slotContainer.innerHTML = slotContainer.getAttribute(
                                    'data-original-content');
                ...
                document.querySelectorAll('.marcar-todos-btn, .marcar-todos-btn-ausente').forEach(b => {
                    b.disabled = false;
                    b.classList.remove('opacity-60');
                });
                document.querySelectorAll('.presenca-btn, .presenca-btn-ausencia, .editar-presenca-btn')
                    .forEach(b => {
                        b.disabled = false;
                    });
            };
            ...
            document.querySelectorAll('.presenca-btn-ausencia').forEach(button => {
                button.addEventListener('click', function() {
                    const alunoId = this.dataset.alunoId;
                    const data = this.dataset.data;
                    const tempoAula = this.dataset.tempoAula || this.getAttribute(
                        'data-tempo-aula');
            ...
            document.getElementById('salvarJustificativa').addEventListener('click', async function() {
                ...
                if (massDiaFContext && massDiaFContext.data == data) {
                    try {
                        ...
                        `;
                            ...
                            const response = await fetch('{{ route('presencas.store.individual') }}', {
                                ...
                                body: JSON.stringify({
                                    ...
                                    escola_id: {{ auth()->user()->escola_id ?? 'null' }},
                                    sala_id: massDiaFContext.salaId || getSalaIdFor(
                                        alunoIdItem, data)
                                })
                            });
                            const resp = await response.json();
                            if (!resp.success) throw new Error(resp.message ||
                                'Falha ao registrar ausência');
                        ...
                        successMsg.className =
                            'fixed top-4 right-4 bg-green-600 text-white px-4 py-2 rounded shadow-lg z-50';
                        successMsg.innerHTML =
                            `<i class="fas fa-check-circle mr-2"></i>Todos marcados como F! (${processedCount}/${totalCount})`;
                        document.body.appendChild(successMsg);
                        setTimeout(() => {
                            successMsg.remove();
                        }, 1500);
                ...
                if (massAlunoFContext && massAlunoFContext.alunoId == alunoId && massAlunoFContext
                    .data == data) {
                    try {
                        let processedCount = 0;
                        ...
                        for (const t of massAlunoFContext.tempos) {
                            ...
                            const slotEntry = massAlunoFContext.slotsMap.find(x => String(x.tempo) ===
                                String(t));
                            const slotsArr = slotEntry ? slotEntry.slots : [];
                            slotsArr.forEach(slotContainer => {
                                if (slotContainer) {
                            ...
                            const response = await fetch('{{ route('presencas.store.individual') }}', {
                                ...
                                body: JSON.stringify({
                                    ...
                                    justificativa: justificativa,
                                    escola_id: {{ auth()->user()->escola_id ?? 'null' }},
                                    sala_id: (massAlunoFContext && massAlunoFContext
                                            .salaId) ? massAlunoFContext.salaId :
                                        getSalaIdFor(alunoId, data)
                                })
                            });
                            const resp = await response.json();
                            if (!resp.success) throw new Error(resp.message ||
                        ...
                        successMsg.className =
                            'fixed top-4 right-4 bg-green-600 text-white px-4 py-2 rounded shadow-lg z-50';
                        successMsg.innerHTML =
                            `<i class="fas fa-check-circle mr-2"></i>Turno marcado como F! (${processedCount}/${totalCount} tempos processados)`;
                        document.body.appendChild(successMsg);
                        setTimeout(() => {
                            successMsg.remove();
                        }, 1500);
                ...
                try {
                    ...
                    if (dataResp.success) {
                        ...
                        const btn = document.querySelector(
                            `.presenca-btn-ausencia[data-aluno-id="${alunoId}"][data-data="${data}"][data-tempo-aula="${tempoAula}"]`
                        );
                        if (btn) {
                            const slotContainer = btn.parentElement;
                            if (slotContainer) {
                        ...
                        successMsg.className =
                            'fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded shadow-lg z-50';
                        successMsg.innerHTML =
                            '<i class="fas fa-check-circle mr-2"></i>Ausência justificada com sucesso!';
                        document.body.appendChild(successMsg);
                        ...
                        alertSystem.error('Erro ao registrar ausência: ' + (dataResp.message ||
                            'Erro desconhecido'));
                    }
                } catch (error) {
                    console.error('Erro:', error);
                    ...
                    fetch('{{ route('presencas.store.individual') }}', {
                        ...
                        `;
                                ...
                                successMsg.className =
                                    'fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded shadow-lg z-50';
                                successMsg.innerHTML =
                                    '<i class="fas fa-check-circle mr-2"></i>Presença registrada com sucesso!';
                                document.body.appendChild(successMsg);
                                ...
                                alertSystem.error('Erro ao registrar presença: ' + (data.message ||
                                    'Erro desconhecido'));
                            }
                        })
                        .catch(error => {
            ...
            if (lancarContainer) {
                ...
                observer.observe(lancarContainer, {
                    childList: true,
                    subtree: true
                });
            }
