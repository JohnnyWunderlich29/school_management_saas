...
@section('content')
    ...
    <x-card>
        ...
        <x-collapsible-filter title="Filtros de Recebimentos" :action="route('admin.recebimentos.index')" :clear-route="route('admin.recebimentos.index')"
            target="recebimentos-table-wrapper">
            <x-filter-field name="status" label="Status" type="select" empty-option="Todos" value="pending"
                :options="$statusOptions" />

            <x-filter-field name="gateway" label="Gateway" type="select" empty-option="Todos" :options="$gatewayOptions" />
            ...
            <div data-ajax-content>
                @if ($recebimentos->count() > 0)
                    <x-table :headers="[
                        ...
                        ['label' => 'Ações'],
                    ]" :actions="false" striped hover responsive sortable :currentSort="request('sort')"
                            ...
                            <x-table-row :index="$loop->index">
                                ...
                                <x-table-cell>
                                    @if (!empty($rec->payer_id) && !empty($rec->payer_nome))
                                        <a href="{{ route('responsaveis.show', $rec->payer_id) }}"
                                            class="text-indigo-600 hover:text-indigo-800 text-sm font-medium"
                                            title="Ver cadastro do responsável">{{ trim(($rec->payer_nome ?? '') . ' ' . ($rec->payer_sobrenome ?? '')) }}</a>
                                    @else
                                ...
                                <x-table-cell>
                                    <div class="flex items-center gap-2 text-sm text-gray-700">
                                        ...
                                        <button type="button" class="text-indigo-600 hover:text-indigo-800"
                                            title="Abrir boleto" onclick="openBoleto({{ $rec->id }})">
                                ...
                                <x-table-cell>
                                    <div class="text-sm font-medium">
                                        ...
                                        <span id="inv-status-{{ $rec->id }}" data-status="{{ $rec->status }}"
                                ...
                                <x-table-cell>
                                    @permission('finance.admin')
                                        <div class="flex items-center gap-2">
                                            <x-button size="sm" color="secondary" title="Visualizar fatura"
                                                onclick="viewInvoiceDetails({{ $rec->id }})">
                                                <i class="fas fa-eye"></i>
                                            </x-button>

                                            @if (in_array($rec->status, ['pending', 'overdue']))
                                                <!-- Reenviar cobrança -->
                                                <x-button size="sm" color="info" title="Reenviar cobrança"
                                                    onclick="openInvAction(this, 'resend', {{ $rec->id }})">
                                                    <i class="fas fa-paper-plane"></i>
                                                </x-button>

                                                <!-- Marcar como pago -->
                                                <x-button size="sm" color="success" title="Marcar como pago"
                                                    onclick="openInvAction(this, 'mark-paid', {{ $rec->id }})">
                                                    <i class="fas fa-check-circle"></i>
                                                </x-button>

                                                <!-- Cancelar fatura -->
                                                <x-button size="sm" color="danger" title="Cancelar fatura"
                                                    onclick="openInvAction(this, 'cancel', {{ $rec->id }})">
                                                    <i class="fas fa-times"></i>
                                                </x-button>
                                            @endif

                                            @if ($rec->status === 'paid')
                                                <x-button size="sm" color="warning" title="Estornar pagamento"
                                                    onclick="openInvAction(this, 'refund', {{ $rec->id }})">
    ...
    <x-modal id="inv-pay-modal" title="Marcar fatura como paga" maxWidth="max-w-lg">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <x-select id="pay-method" name="pay-method" label="Método" :options="[
                    'cash' => 'Dinheiro',
                    'pix' => 'PIX',
            ...
            <div>
                <x-textarea id="pay-description" name="pay-description" label="Descrição" rows="3"
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-2">
                ...
                <x-button id="pay-confirm" color="success"><i class="fas fa-check mr-2"></i>Confirmar
                    pagamento</x-button>
            </div>
        </x-slot>
    </x-modal>
    @superadmin
        <x-card class="mt-6">
            ...
            <div id="request-logs"
                class="text-sm font-mono whitespace-pre-wrap bg-gray-50 p-3 rounded border border-gray-200 h-48 overflow-auto">
            </div>
        </x-card>
    @endsuperadmin
    ...
    @push('scripts')
        <script>
            ...
            function setStatusBadge(invoiceId, newStatus) {
                ...
                if (!el) return;
                el.classList.remove('bg-yellow-100', 'text-yellow-800', 'bg-green-100', 'text-green-800', 'bg-red-100',
                    'text-red-800', 'bg-gray-100', 'text-gray-800');
            ...
            async function viewInvoiceDetails(invoiceId) {
                ...
                try {
                    ...
                    if (extraEl) {
                        ...
                        if (data.boleto_url) {
                            html +=
                                `<div><a href="${data.boleto_url}" target="_blank" class="text-blue-600 hover:text-blue-800">Ver Boleto</a></div>`;
                        }
                        if (data.pix_qr_code) {
                            html +=
                                `<div><strong>PIX QR Code:</strong><br><img src="data:image/png;base64,${data.pix_qr_code}" class="mt-2 max-w-xs"></div>`;
                        }
                        extraEl.innerHTML = html;
                    }
                    appendLog('Visualizou detalhes', {
                        invoice: data.number,
                        status: data.status
                    });
                } catch (error) {
                ...
                try {
                    ...
                    if (data && data.boleto_url) {
                        ...
                        if (typeof appendLog === 'function') {
                            appendLog('Abriu boleto', {
                                invoice: data.number || invoiceId
                            });
                        }
                    } else {
            ...
            function openInvAction(el, action, invoiceId) {
                ...
                if (btn) {
                    ...
                    btn.disabled = false;
                    const remove = ['bg-red-600', 'hover:bg-red-700', 'bg-green-600', 'hover:bg-green-700', 'bg-yellow-500',
                        'hover:bg-yellow-600', 'bg-blue-600', 'hover:bg-blue-700'
                    ];
            ...
            async function updateRecebimentosContainer(url) {
                ...
                try {
                    ...
                    const newContainer = doc.getElementById('recebimentos-table-wrapper') || doc.querySelector(
                        '[data-target="recebimentos-table-wrapper"]');
                    if (newContainer) {
            ...
            async function prefillPayModal(invoiceId) {
                try {
                    ...
                    appendLog('Abriu modal de pagamento', {
                        invoice: data.number || invoiceId
                    });
                } catch (e) {
                ...
                btn.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        if (response.ok && (data.success ?? true)) {
                            ...
                            appendLog('Falha ao confirmar pagamento', {
                                invoice_id: invoiceId,
