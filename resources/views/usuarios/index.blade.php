    ...
    <x-card>
        ...
        <div id="usuarios-list-wrapper" class="relative">
            ...
            <div data-ajax-content>
                <!-- Tabela desktop -->
                <x-table class="hidden md:block" :headers="[
                    ['label' => 'ID', 'sort' => 'id'],
                            ...
                            <div class="w-full overflow-hidden">
                                <div class="flex flex-col grid grid-cols-2 gap-2">
                                    ...
                                    @permission('usuarios.excluir')
                                        <button type="button"
                                            onclick="confirmToggleStatus('{{ $user->id }}', '{{ $user->name }}', '{{ $user->ativo ? 'inativar' : 'ativar' }}', '{{ route('usuarios.toggle-status', $user) }}')"
                                            class="flex-1 col-span-2 {{ $user->ativo ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700' }} text-white text-center py-3 px-4 rounded-lg font-medium text-sm min-h-[48px] flex items-center justify-center transition-colors"
                                            @if ($user->id === auth()->id()) disabled @endif>
