    ...
    <div id="wizard-container" class="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <nav class="flex space-x-8 overflow-x-auto">
                @php
                    $steps = [
                        ...
                        4 => ['title' => 'Período', 'icon' => 'fas fa-calendar', 'description' => 'Duração e cronograma'],
                        5 => ['title' => 'Conteúdo', 'icon' => 'fas fa-book', 'description' => 'Objetivos e metodologia'],
                ...
                @foreach($steps as $stepNumber => $step)
                    <div class="flex items-center space-x-3 step-nav {{ $stepNumber === $initialStep ? 'active' : '' }}" data-step="{{ $stepNumber }}">
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
            <div class="flex justify-between items-center gap-2 flex-wrap">
                <div>
                    <button type="button" id="btn-previous" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed" disabled>
                ...
                <div class="flex space-x-3 flex-wrap">
                    ...
                    <button type="button" id="btn-next" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
...
class PlanejamentoWizard {
    ...
    async loadStep(step) {
        try {
            let url = `{{ route('planejamentos.wizard.step', ['step' => '__STEP__']) }}`.replace('__STEP__', step);
            ...
            if (!response.ok) {
                ...
                throw new Error(`Erro ao carregar etapa: ${response.status} - ${response.statusText}`);
            }
            
            const html = await response.text();
            console.log('[Wizard] loadStep URL:', url);
            ...
            if (!html || html.trim().length === 0) {
                ...
                document.getElementById('step-content').innerHTML = `
                    <div class=\"text-center py-8\">
                        <p class=\"text-red-600 font-semibold mb-2\">N�o foi poss�vel carregar o conte�do da etapa.</p>
            ...
            if (currentForm) {
                const updateData = () => {
                    try {
                        ...
                        if (this.currentStep === 5) { const existente = this.formData[5] || {}; this.formData[5] = { ...existente, ...data }; } else { this.formData[this.currentStep] = data; }
            ...
            // Exibir feedback na pr�pria �rea de conte�do para evitar loader infinito
            document.getElementById('step-content').innerHTML = `
                <div class=\"text-center py-8\">
            ...
            `;
            this.showError('Erro ao carregar etapa. Tente novamente.');
        }
    }
        ...
        scripts.forEach((script, index) => {
            if (script.src) {
                ...
                try {
                    ...
                    if (!scriptContent) {
                        console.warn(`Script inline ${index + 1} está vazio`);
                        return;
                    }
                    ...
                    if (openBraces !== closeBraces) {
                        console.warn(`Script inline ${index + 1} tem chaves possivelmente desbalanceadas: ${openBraces} aberturas, ${closeBraces} fechamentos`);
                    ...
                    try {
                        ...
                        console.warn(`Poss�vel erro de sintaxe no script inline ${index + 1}:`, syntaxError);
                        console.warn('Conteúdo do script:', scriptContent);
                    ...
                    console.error(`Erro ao executar script inline ${index + 1}:`, error);
    ...
    updateUI() {
        ...
        document.getElementById('step-counter').textContent = `Etapa ${this.currentStep} de ${this.maxSteps}`;
        ...
        document.getElementById('btn-previous').disabled = (this.currentStep === 1) || (isEditing && this.currentStep <= 5);
        ...
        try {
            ...
            const result = await response.json();
             ...
             if (this.currentStep === 5) {
                 ...
                 this.formData[5] = { ...existing, ...formData };
             } else {
    ...
    async saveDraft() {
        try {
            ...
            if (this.currentStep === 5) {
                const existing = this.formData[5] || {};
                allData = { ...this.formData, 5: { ...existing, ...this.collectStepData() } };
            } else {
                allData = { ...this.formData, [this.currentStep]: this.collectStepData() };
            }
            ...
            if (result.success) {
                ...
                if (result.planejamento_id && !this.planejamento) {
                    this.planejamento = { id: result.planejamento_id };
                }
            } else {
    ...
    async finishWizard() {
        try {
            ...
            const termosEl = document.querySelector('input[name="aceita_termos"], input[name="aceitar_termos"], #checkbox_termos');
            ...
            const tryGetFromStores = (key) => {
                ...
                if (this.planejamento) {
                    ...
                    // IDs diretos
                    if (key === 'turma_id' && this.planejamento.turma_id != null) return this.planejamento.turma_id;
                    // Fallback via relacionamento carregado
                    if (key === 'turma_id' && this.planejamento.turma && this.planejamento.turma.id != null) return this.planejamento.turma.id;
                    if (key === 'disciplina_id' && this.planejamento.disciplina_id != null) return this.planejamento.disciplina_id;
                    // Fallback via relacionamento carregado
                    if (key === 'disciplina_id' && this.planejamento.disciplina && this.planejamento.disciplina.id != null) return this.planejamento.disciplina.id;
                    if (key === 'professor_id') {
                        const p = this.planejamento.professor_id ?? this.planejamento.user_id;
                        if (p != null && String(p) !== '') return p;
                    }
                    // Período e duração
                    if (key === 'data_inicio' && this.planejamento.data_inicio) return this.planejamento.data_inicio;
                    if (key === 'data_fim' && this.planejamento.data_fim) return this.planejamento.data_fim;
                    if (key === 'numero_dias' && this.planejamento.numero_dias) return this.planejamento.numero_dias;
                    if (key === 'tipo_periodo') {
                    ...
                    if (key === 'aulas_por_semana' && this.planejamento.aulas_por_semana != null) return this.planejamento.aulas_por_semana;
                    if (key === 'carga_horaria_aula' && this.planejamento.carga_horaria_aula != null) return this.planejamento.carga_horaria_aula;
                }
                // 2) window.wizardData stepN
                if (window.wizardData) {
                    for (let i = 1; i <= 6; i++) {
                        const s = window.wizardData[`step${i}`];
                ...
                if (window.planejamentoWizard && window.planejamentoWizard.formData) {
                    ...
                    if (window.planejamentoWizard.formData[key] != null) return window.planejamentoWizard.formData[key];
                }

                // 5.1) Fallback do período vindo da Step 5 (preview)
                if (key === 'data_inicio' && window.planejamentoPeriodo && window.planejamentoPeriodo.inicio) {
            ...
            arrayKeys.forEach((k) => {
                const v = flattened[k];
                if (typeof v === 'string') {
                    try { flattened[k] = JSON.parse(v); } catch (_) {}
                }
            });

            // 6) Campos de período obrigatórios
            const periodKeys = ['aulas_por_semana', 'carga_horaria_aula', 'data_inicio', 'data_fim', 'numero_dias', 'bimestre', 'ano_letivo'];
            ...
            const diaKeys = () => {
                ...
                try {
                    if (flattened.planejamentos_diarios && typeof flattened.planejamentos_diarios === 'object') {
                        Object.entries(flattened.planejamentos_diarios).forEach(([d, ok]) => { if (ok) diasPlanejados.push(d); });
                    }
                    if (diasPlanejados.length === 0 && flattened.dados_por_dia && typeof flattened.dados_por_dia === 'object') {
            ...
            if ((!flattened.data_inicio || !flattened.data_fim || !flattened.numero_dias) && typeof diaKeys === 'function') {
            ...
            if ((!flattened.turno_id || flattened.turno_id === '') && this.planejamento) {
                ...
                const modalidadeId = flattened.modalidade_ensino_id || this.planejamento.modalidade_ensino_id || this.planejamento.modalidade;
                if (turnoStr && modalidadeId) {
                    try {
                        const turnosUrl = '{{ route("planejamentos.turnos-disponiveis") }}' + `?modalidade_id=${encodeURIComponent(modalidadeId)}`;
                        const respTurnos = await fetch(turnosUrl, { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
                        if (respTurnos.ok) {
                            const lista = await respTurnos.json();
                            if (Array.isArray(lista)) {
                                const match = lista.find(t => (t.value || '').toLowerCase() === turnoStr || (t.label || '').toLowerCase() === turnoStr);
                                if (match && match.id) {
            ...
            try {
                ...
                const dailyStatusObj = (this.formData?.[5]?.planejamentos_diarios) || (typeof flattened.planejamentos_diarios === 'object' && !Array.isArray(flattened.planejamentos_diarios) ? flattened.planejamentos_diarios : {});

                if (!Array.isArray(flattened.planejamentos_diarios)) {
                    const diarios = Object.entries(dailyMap).map(([dateKey, v]) => {
                        let dt;
                        try { dt = new Date(dateKey); } catch (_) { dt = null; }
                        return {
                            data: dateKey,
                            ...
                            saberes_conhecimentos: (typeof v?.saberes_conhecimentos === 'string') ? v.saberes_conhecimentos : (Array.isArray(v?.saberes_conhecimentos) ? v.saberes_conhecimentos : (Array.isArray(v?.saberes_selecionados) ? v.saberes_selecionados : '')),
                            objetivos_especificos: (typeof v?.objetivos_especificos === 'string') ? v.objetivos_especificos : '',
                    ...
                    const globCampos = Array.isArray(flattened.campos_experiencia) ? flattened.campos_experiencia : [];
                    const globObjApr = Array.isArray(flattened.objetivos_aprendizagem) ? flattened.objetivos_aprendizagem : [];
                    const globSaberes = (typeof flattened.saberes_conhecimentos === 'string' && flattened.saberes_conhecimentos.trim().length > 0)
                        ? flattened.saberes_conhecimentos
                        : (Array.isArray(flattened.saberes_conhecimentos) && flattened.saberes_conhecimentos.length > 0)
                            ? flattened.saberes_conhecimentos
                            : (Array.isArray(flattened.saberes_selecionados) && flattened.saberes_selecionados.length > 0)
                                ? flattened.saberes_selecionados
                                : '';
                    flattened.planejamentos_diarios = flattened.planejamentos_diarios.map(d => ({
                        ...d,
                        campos_experiencia: (Array.isArray(d.campos_experiencia) && d.campos_experiencia.length > 0) ? d.campos_experiencia : globCampos,
                        objetivos_aprendizagem: (Array.isArray(d.objetivos_aprendizagem) && d.objetivos_aprendizagem.length > 0) ? d.objetivos_aprendizagem : globObjApr,
                        saberes_conhecimentos: ((typeof d.saberes_conhecimentos === 'string' && d.saberes_conhecimentos.trim().length > 0)
                            || (Array.isArray(d.saberes_conhecimentos) && d.saberes_conhecimentos.length > 0)) ? d.saberes_conhecimentos : globSaberes
                    }));
                }

                if ((!flattened.numero_dias || Number(flattened.numero_dias) <= 0) && Array.isArray(flattened.planejamentos_diarios)) {
                    const datasUnicas = [...new Set(flattened.planejamentos_diarios.map(d => d.data).filter(Boolean))];
            ...
            if (result.success) {
                 ...
                 setTimeout(() => {
                     ...
                     const shouldGoToIndex = (flattened?.save_as_draft === true) || ((flattened?.status || '').toLowerCase() === 'rascunho');
                     const targetUrl = shouldGoToIndex ? '{{ route("planejamentos.index") }}' : (result.redirect_url || viewUrl);
    ...
    showError(message) {
        ...
        if (container) {
            container.insertAdjacentHTML('beforeend', `<div class="mt-3 p-3 bg-red-100 text-red-800 rounded">Erro: ${message}</div>`);
        } else {
            alert('Erro: ' + message);
        }
    }
    ...
    showSuccess(message) {
        ...
        const container = document.querySelector('#step-content');
        if (container) {
            container.insertAdjacentHTML('beforeend', `<div class="mt-3 p-3 bg-green-100 text-green-800 rounded">Sucesso: ${message}</div>`);
        }
    }
