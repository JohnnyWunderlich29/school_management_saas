...
<div x-data="{
    ...
    confirm() {
     ...
     x-show="open"
     x-on:show-confirmation.window="show($event.detail)"
    ...
    <!-- Background overlay -->
    <div class="fixed inset-0 bg-transparent transition-opacity"
         x-show="open"
         x-transition:enter="ease-out duration-300"
         x-transition:enter-start="opacity-0"
         x-transition:enter-end="opacity-100"
        ...
        <div x-show="open"
            ...
            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    <!-- Icon -->
                    <div class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full {{ $confirmColor === 'red' ? 'bg-red-100' : ($confirmColor === 'green' ? 'bg-green-100' : 'bg-blue-100') }} sm:mx-0 sm:h-10 sm:w-10">
                        @if($confirmColor === 'red')
                    ...
                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                        ...
                        <div class="mt-2">
                            ...
                            <div class="mt-4">
                                ...
                                @endif
                                <textarea x-ref="modalInput"
                                         x-model="inputValue"
                                         @if($inputRequired) required @endif
                                         class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                         rows="3"
                                         placeholder="{{ $inputPlaceholder }}"
                                         x-on:keydown.enter.ctrl="confirm()"></textarea>
                                @if($inputRequired)
...
window.showConfirmation = function(options = {}) {
    window.dispatchEvent(new CustomEvent('show-confirmation', { detail: options }));
};
