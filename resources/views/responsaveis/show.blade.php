    ...
    <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
        <div class="container md:col-span-2 gap-4 mx-auto w-full px-2">
            <x-card class="flex flex-col">
                ...
                <div class="flex flex-col justify-between items-center mb-6">
                    ...
                    <div class="flex flex-col w-full mt-6 pt-4 border-t md:flex-row">
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <x-button href="{{ route('responsaveis.edit', $responsavel->id) }}" color="primary"
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <span
                                        class="font-medium text-mobile-body">{{ $responsavel->rg ?? 'Não informado' }}</span>
                                </div>
                            </div>
                            <div>
                                <span class="text-gray-500 block text-mobile-body">Gênero:</span>
                                <span
                                    class="font-medium text-mobile-body">{{ $responsavel->genero ?? 'Não informado' }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <span
                                    class="font-medium text-mobile-body">{{ $responsavel->email ?? 'Não informado' }}</span>
                            </div>
                            <div>
                ...
                <!-- Configurações Financeiras Desktop -->
                <div class="hidden md:block mt-6 bg-gray-50 p-4 rounded-lg">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">
                        <i class="fas fa-file-invoice-dollar mr-2"></i>Configurações de Faturamento
                    </h3>
                    <div class="flex items-center justify-between p-3 bg-white rounded border">
                        <div>
                            <p class="font-medium text-gray-900">Consolidar faturas?</p>
                            <p class="text-xs text-gray-500">Agrupa várias mensalidades em uma cobrança.</p>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" id="toggle-consolidation" @checked($responsavel->consolidate_billing)
                                class="sr-only peer">
                            <div
                ...
                <div class="md:hidden mt-4 space-y-4">
                    <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                        <div class="flex items-center mb-3">
                            ...
                            <h3 class="text-mobile-title text-gray-900">Financeiro</h3>
                        </div>
                        <div class="flex items-center justify-between">
                            <div>
                                <span class="text-gray-500 block text-mobile-body">Consolidar Faturas?</span>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" id="toggle-consolidation-mobile" @checked($responsavel->consolidate_billing)
                                    class="sr-only peer">
                                <div
                                    class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-3 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600">
                                </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const toggleDesktop = document.getElementById('toggle-consolidation');
            const toggleMobile = document.getElementById('toggle-consolidation-mobile');

            async function toggleConsolidation(e) {
                const checkbox = e.target;
                const originalState = !checkbox.checked;

                try {
                    const response = await fetch(
                        "{{ route('responsaveis.toggle-consolidation', $responsavel->id) }}", {
                            method: 'PATCH',
                            headers: {
                    ...
                    if (toggleDesktop) toggleDesktop.checked = data.consolidate_billing;
                    if (toggleMobile) toggleMobile.checked = data.consolidate_billing;

                    // Alerta de sucesso (se houver AlertService global ou similar no layout)
                    ...
                    console.error(error);
                    checkbox.checked = originalState; // Reverter estadao em caso de erro
                    alert('Erro ao atualizar configuração. Tente novamente.');
                }
            }

            if (toggleDesktop) toggleDesktop.addEventListener('change', toggleConsolidation);
            if (toggleMobile) toggleMobile.addEventListener('change', toggleConsolidation);
        });
    </script>
@endsection
@endsection
