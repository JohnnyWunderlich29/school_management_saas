...
@section('content')
    <x-breadcrumbs :items="[['title' => 'Responsáveis', 'url' => route('responsaveis.index')], ['title' => 'Editar Responsável']]" />

    <div class="bg-white rounded-lg shadow-sm p-6">
        <form action="{{ route('responsaveis.update', $responsavel->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <!-- Informações Pessoais -->
            <div>
                <h4 class="text-md font-semibold text-gray-800 mb-4">Informações Pessoais</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <x-input type="text" name="nome" label="Nome *" :value="old('nome') ?? $responsavel->nome" required
                            placeholder="Nome do responsável" />
                    </div>

                    <div>
                        <x-input type="text" name="sobrenome" label="Sobrenome *" :value="old('sobrenome') ?? $responsavel->sobrenome" required
                            placeholder="Sobrenome do responsável" />
                    </div>

                    <div>
                        <x-input type="text" name="cpf" label="CPF *" :value="old('cpf') ?? $responsavel->cpf" required
                            placeholder="000.000.000-00" id="cpf" />
                    </div>

                    <div>
                        <x-input type="text" name="rg" label="RG" :value="old('rg') ?? $responsavel->rg"
                            placeholder="00.000.000-0" />
                    </div>

                    <div>
                        <x-input type="text" name="data_nascimento" label="Data de Nascimento" :value="old('data_nascimento') ??
                            ($responsavel->data_nascimento ? $responsavel->data_nascimento->format('d/m/Y') : '')"
                    ...
                    <div>
                        <x-select name="genero" label="Gênero">
                            ...
                            <option value="Masculino"
                                {{ (old('genero') ?? $responsavel->genero) == 'Masculino' ? 'selected' : '' }}>Masculino
                            </option>
                            <option value="Feminino"
                                {{ (old('genero') ?? $responsavel->genero) == 'Feminino' ? 'selected' : '' }}>Feminino
                            </option>
                            <option value="Outro"
                                {{ (old('genero') ?? $responsavel->genero) == 'Outro' ? 'selected' : '' }}>Outro</option>
                        </x-select>
                    </div>
                </div>
            </div>
            ...
            <div>
                <h4 class="text-md font-semibold text-gray-800 mb-4">Informações de Contato</h4>
                <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                    <div>
                        <x-input type="tel" name="telefone_principal" label="Telefone *" :value="old('telefone_principal') ?? $responsavel->telefone_principal" required
                            placeholder="(00) 00000-0000" id="telefone_principal" />
                    </div>

                    <div>
                        <x-input type="tel" name="telefone_secundario" label="Telefone Secundário" :value="old('telefone_secundario') ?? $responsavel->telefone_secundario"
                            placeholder="(00) 00000-0000" />
                    </div>

                    <div>
                        <x-input type="email" name="email" label="E-mail" :value="old('email') ?? $responsavel->email"
            ...
            <div>
                <h4 class="text-md font-semibold text-gray-800 mb-4">Informações de Endereço</h4>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        <x-input type="text" name="endereco" label="Endereço" :value="old('endereco') ?? $responsavel->endereco"
                            placeholder="Rua, número, complemento" id="endereco" />
                    </div>

                    <div>
                        <x-input type="text" name="cidade" label="Cidade" :value="old('cidade') ?? $responsavel->cidade"
                            placeholder="Nome da cidade" id="cidade" />
                    </div>

                    <div>
                        <x-input type="text" name="estado" label="Estado" :value="old('estado') ?? $responsavel->estado" placeholder="UF"
            ...
            <!-- Parentesco -->
            <div>
                <h4 class="text-md font-semibold text-gray-800 mb-4">Parentesco</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <x-select name="parentesco" label="Qual o parentesco *" required>
                            <option value="">Selecione o parentesco</option>
                            <option value="Pai"
                                {{ (old('parentesco') ?? $responsavel->parentesco) == 'Pai' ? 'selected' : '' }}>Pai
                            </option>
                            <option value="Mãe"
                                {{ (old('parentesco') ?? $responsavel->parentesco) == 'Mãe' ? 'selected' : '' }}>Mãe
                            </option>
                            <option value="Avô"
                                {{ (old('parentesco') ?? $responsavel->parentesco) == 'Avô' ? 'selected' : '' }}>Avô
                            </option>
                            <option value="Avó"
                                {{ (old('parentesco') ?? $responsavel->parentesco) == 'Avó' ? 'selected' : '' }}>Avó
                            </option>
                            <option value="Tio"
                                {{ (old('parentesco') ?? $responsavel->parentesco) == 'Tio' ? 'selected' : '' }}>Tio
                            </option>
                            <option value="Tia"
                                {{ (old('parentesco') ?? $responsavel->parentesco) == 'Tia' ? 'selected' : '' }}>Tia
                            </option>
                            <option value="Responsável Legal"
                                {{ (old('parentesco') ?? $responsavel->parentesco) == 'Responsável Legal' ? 'selected' : '' }}>
                                Responsável Legal</option>
                            <option value="Outro"
                                {{ (old('parentesco') ?? $responsavel->parentesco) == 'Outro' ? 'selected' : '' }}>Outro
                            </option>
                        </x-select>
                    </div>
                </div>
            ...
            <!-- Configurações -->
            <div>
                <h4 class="text-md font-semibold text-gray-800 mb-4">Configurações</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="flex flex-row gap-4">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="contato_emergencia" value="1"
                                {{ old('contato_emergencia') ?? $responsavel->contato_emergencia ? 'checked' : '' }}
                                class="sr-only peer" />
                            <div
                    ...
                    <div class="flex flex-row gap-4">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="autorizado_buscar" value="1"
                                {{ old('autorizado_buscar') ?? $responsavel->autorizado_buscar ? 'checked' : '' }}
                                class="sr-only peer" />
                            <div
                                class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-3 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600">
                            </div>
            ...
            <div>
                ...
                <div>
                    <x-textarea name="observacoes" label="Observações" :value="old('observacoes') ?? $responsavel->observacoes"
                        placeholder="Observações adicionais sobre o responsável" rows="3" />
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-4 border-t">
                ...
                <x-button type="submit" color="primary" class="w-full sm:w-auto">
                    ...
                    <span class="hidden sm:inline">Atualizar Responsável</span>
