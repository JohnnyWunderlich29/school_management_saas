...
<x-breadcrumbs :items="[
    ...
    ['title' => $comunicado->titulo, 'route' => 'comunicados.show', 'params' => $comunicado->id],
    ['title' => 'Editar']
]" />
...
<x-card>
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600">Edite as informações do comunicado</p>
    </div>

    <form action="{{ route('comunicados.update', $comunicado) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select id="tipo" 
                ...
                @foreach($tipos as $tipo)
                    <option value="{{ $tipo }}" {{ old('tipo', $comunicado->tipo) == $tipo ? 'selected' : '' }}>
                        {{ ucfirst($tipo) }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="destinatario_tipo" 
                ...
                <option value="todos" {{ old('destinatario_tipo', $comunicado->destinatario_tipo) == 'todos' ? 'selected' : '' }}>Todos</option>
                <option value="pais" {{ old('destinatario_tipo', $comunicado->destinatario_tipo) == 'pais' ? 'selected' : '' }}>Pais/Responsáveis</option>
                <option value="professores" {{ old('destinatario_tipo', $comunicado->destinatario_tipo) == 'professores' ? 'selected' : '' }}>Professores</option>
                <option value="turma_especifica" {{ old('destinatario_tipo', $comunicado->destinatario_tipo) == 'turma_especifica' ? 'selected' : '' }}>Turma Específica</option>
            </select>
        ...
        <!-- Turma (condicional) -->
        <div id="turma-field" class="{{ old('destinatario_tipo', $comunicado->destinatario_tipo) == 'turma_especifica' ? '' : 'hidden' }}">
            ...
            <select id="turma_id" 
                ...
                @foreach($turmas as $turma)
                    <option value="{{ $turma->id }}" {{ old('turma_id', $comunicado->turma_id) == $turma->id ? 'selected' : '' }}>
                        {{ $turma->nome }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea id="conteudo" 
                      ...
                      required>{{ old('conteudo', $comunicado->conteudo) }}</textarea>
            @error('conteudo')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="datetime-local" 
                   id="data_evento" 
                   name="data_evento" 
                   value="{{ old('data_evento', $comunicado->data_evento ? $comunicado->data_evento->format('Y-m-d\TH:i') : '') }}"
        ...
        <div class="space-y-4">
            <div class="flex items-center">
                <input type="checkbox" 
                       ...
                       value="1"
                       {{ old('requer_confirmacao', $comunicado->requer_confirmacao) ? 'checked' : '' }}
        ...
        <div class="bg-gray-50 rounded-lg p-4">
            ...
            @if($comunicado->isPublicado())
                <div class="flex items-center text-green-600">
                    ...
                    <span class="text-sm">Publicado em {{ $comunicado->publicado_em->format('d/m/Y H:i') }}</span>
                </div>
            @else
                <div class="flex items-center text-yellow-600">
                    <i class="fas fa-clock mr-2"></i>
                    <span class="text-sm">Rascunho - Não publicado</span>
                </div>
            @endif
        </div>
        ...
        <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
            <x-button href="{{ route('comunicados.show', $comunicado) }}" color="secondary">
                Cancelar
            </x-button>
