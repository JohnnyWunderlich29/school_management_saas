...
@section('content')
    <x-breadcrumbs :items="[['title' => 'Funcionários', 'url' => route('funcionarios.index')], ['title' => 'Editar Funcionário']]" />

    <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6">
        <form action="{{ route('funcionarios.update', $funcionario->id) }}" method="POST" class="space-y-6"
            id="funcionario-edit-form">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <x-input type="text" name="nome" label="Nome *" :value="old('nome', $funcionario->nome)" required
                            placeholder="Digite o nome" />
                    </div>

                    <div>
                        <x-input type="text" name="sobrenome" label="Sobrenome *" :value="old('sobrenome', $funcionario->sobrenome)" required
                            placeholder="Digite o sobrenome" />
                    </div>

                    <div>
                        <x-input type="date" name="data_nascimento" label="Data de Nascimento *" :value="old('data_nascimento', $funcionario->data_nascimento?->format('Y-m-d'))"
                            required />
                    </div>

                    <div>
                        <x-input type="text" name="cpf" label="CPF" :value="old('cpf', $funcionario->cpf)" placeholder="000.000.000-00"
                            id="cpf" />
                    </div>

                    <div>
                        <x-input type="text" name="rg" label="RG" :value="old('rg', $funcionario->rg)" placeholder="Digite o RG" />
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <x-input type="tel" name="telefone" label="Telefone *" :value="old('telefone', $funcionario->telefone)" required
                            placeholder="(00) 00000-0000" id="telefone_funcionario" />
                    </div>

                    <div>
                        <x-input type="email" name="email" label="E-mail *" :value="old('email', $funcionario->email)" required
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        <x-input type="text" name="cep" label="CEP" :value="old('cep', $funcionario->cep)" placeholder="00000-000"
                            id="cep" maxlength="9" class="pr-12" help="Digite o CEP e clique na lupa para buscar.">
                            <button type="button" id="btn-buscar-cep-func-edit"
                                ...
                                <svg id="icon-cep-func-edit" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                    ...
                    <div class="sm:col-span-2 lg:col-span-2">
                        <x-input type="text" name="endereco" label="Endereço" :value="old('endereco', $funcionario->endereco)"
                    ...
                    {{--
                                ...
                                --}}
                    <div>
                        <x-input type="text" name="cidade" label="Cidade" :value="old('cidade', $funcionario->cidade)"
                            placeholder="Digite a cidade" id="cidade" />
                    </div>

                    <div>
                        <x-input type="text" name="estado" label="Estado" :value="old('estado', $funcionario->estado)" placeholder="UF"
                            maxlength="2" id="estado" />
                    </div>

                    <!-- Número -->
                    {{-- <div>
                        <x-input type="text" name="numero" label="Número" :value="old('numero', $funcionario->numero)" placeholder="Número"
                            id="numero" />
                    </div> --}}
                </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <x-select name="cargo" id="cargo"
                            ...
                            @foreach ($cargos as $cargo)
                                <option value="{{ $cargo->nome }}"
                                    {{ old('cargo', $funcionario->cargo) == $cargo->nome ? 'selected' : '' }}>
                                    {{ $cargo->nome }}
                                </option>
                            @endforeach
                        </x-select>
                    ...
                    <div>
                        <x-input type="text" name="departamento" label="Departamento" :value="old('departamento', $funcionario->departamento)"
                            placeholder="Digite o departamento" />
                    </div>

                    <div>
                        <x-input type="date" name="data_contratacao" label="Data de Contratação *" :value="old('data_contratacao', $funcionario->data_contratacao?->format('Y-m-d'))"
                            required />
                    </div>

                    <div>
                        <x-input type="date" name="data_demissao" label="Data de Demissão" :value="old('data_demissao', $funcionario->data_demissao?->format('Y-m-d'))" />
                    </div>

                    <div>
                        <x-input type="number" name="salario" label="Salário" :value="old('salario', $funcionario->salario)" placeholder="0.00"
                            step="0.01" />
                    </div>

                    <div class="flex items-center">
                        <input type="checkbox" name="ativo" id="ativo" class="mr-2" value="1"
            ...
            @if ($funcionario->user_id)
                <div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="atualizar_usuario" id="atualizar_usuario" class="mr-2"
                                value="1" {{ old('atualizar_usuario') ? 'checked' : '' }}>
                            <label for="atualizar_usuario" class="text-sm text-gray-700">Atualizar senha do
                                usuário</label>
                        </div>

                        <div id="usuario_fields" class="grid grid-cols-1 sm:grid-cols-2 gap-4"
                            style="display: {{ old('atualizar_usuario') ? 'grid' : 'none' }};">
                            <div>
                                <x-input type="password" name="password" label="Nova Senha"
                                    placeholder="Digite a nova senha" />
                            </div>

                            <div>
                                <x-input type="password" name="password_confirmation" label="Confirmar Nova Senha"
            ...
            <div>
                ...
                <div>
                    <x-textarea name="observacoes" label="Observações" :value="old('observacoes', $funcionario->observacoes)"
                        placeholder="Observações adicionais sobre o funcionário" rows="3" />
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-4 border-t">
                ...
                <x-button type="submit" color="primary" class="w-full sm:w-auto">
                    <i class="fas fa-save mr-1"></i> <span class="hidden sm:inline">Atualizar Funcionário</span><span
                        class="sm:hidden">Atualizar</span>
                </x-button>
            </div>
        </form>
