        ...
        <form action="{{ route('alunos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div class="relative" id="sala-select-container">
                            ...
                            <div id="sala-dropdown" class="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg hidden max-h-60 overflow-y-auto">
                                <div class="py-1">
                                    ...
                                    @foreach ($salas as $index => $sala)
                                        <div class="sala-option px-3 py-2 cursor-pointer hover:bg-gray-100 {{ $index >= 8 ? 'hidden' : '' }}" 
                                            ...
                                            <div class="flex justify-between items-center">
                                                <span class="text-sm text-gray-900">{{ $sala->codigo }} - {{ $sala->nome }}</span>
                                                <span class="text-xs text-gray-500">({{ $sala->alunos->count() }}/{{ $sala->capacidade }})</span>
                                            </div>
                                        </div>
                                    @endforeach
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                        <x-input type="text" name="cep" id="cep" label="CEP" :value="old('cep')" placeholder="00000-000" maxlength="9" class="pr-12" help="Digite o CEP e clique na lupa para buscar.">
                            <button type="button" id="btn-buscar-cep-aluno" class="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-indigo-600" title="Buscar CEP" aria-label="Buscar CEP">
                                <svg id="icon-cep-aluno" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div>
                        ...
                        <!-- Resultados da busca -->
                        <div id="search-results" class="mt-2 hidden bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto"></div>
                    </div>
                    ...
                    <div>
                        ...
                        <div id="responsaveis-selecionados" class="min-h-[100px] border border-gray-300 rounded-md p-3 bg-gray-50">
            ...
            <div>
                ...
                <div class="space-y-4">
                    <!-- Upload Area -->
                    <div class="relative">
                        <div id="upload-area" class="border-2 border-dashed border-gray-300 rounded-lg p-4 sm:p-8 text-center hover:border-indigo-400 transition-colors duration-200 cursor-pointer bg-gray-50 hover:bg-gray-100">
                            <div class="space-y-3 sm:space-y-4">
                                ...
                                <div>
                                    <p class="text-base sm:text-lg font-medium text-gray-900">Clique para selecionar arquivos</p>
            ...
            <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-4 border-t">
                ...
                <x-button type="submit" color="primary" class="w-full sm:w-auto">
                    <i class="fas fa-save mr-1"></i> <span class="hidden sm:inline">Salvar Aluno</span><span class="sm:hidden">Salvar</span>
                </x-button>
            </div>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function atualizarInterface() {
        ...
        if (responsaveisSelecionados.length === 0) {
            responsaveisSelecionadosDiv.innerHTML = '<p class="text-sm text-gray-500 text-center">Nenhum responsável selecionado</p>';
        } else {
        ...
        responsavelPrincipalSelect.innerHTML = '<option value="">Selecione um responsável principal...</option>';
